...
public class UniKVServiceStubManager implements InvocationService {
    ...
    @Inject
    public UniKVServiceStubManager(@Named(CONFIG_KEY) NanoKVClientConfig config, ChannelManager channelManager) {
    ...
    void handleRpcResult(CompletableFuture<KVResponse> future, ListenableFuture<KVResponse> rawFuture) {
        try {
            ...
            future.completeExceptionally(new NanoKVClientException(getRootCause(e)));
        }
    }
