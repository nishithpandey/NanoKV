...
import io.microraft.nanokv.client.kv.Ordered;
import io.microraft.nanokv.internal.utils.Exceptions;
...
public class KVTest extends BaseTest {
    ...
    private static final List<NanoKV> servers = new ArrayList<>();

    private final boolean singleConnection;
    private NanoKVClient client;
    private KV kv;
    ...
    public void initClient() {
        ...
        NanoKVClientConfig clientConfig = NanoKVClientConfig.newBuilder().setConfig(config)
                .setSingleConnection(singleConnection).build();
        client = NanoKVClient.newInstance(clientConfig);
        kv = client.getKV();
        kv.clear();
    }
