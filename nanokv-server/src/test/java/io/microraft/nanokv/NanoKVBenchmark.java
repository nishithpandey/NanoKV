...
@Fork(1)
public class NanoKVBenchmark {

    public static void main(String[] args) throws RunnerException {
        Options opt = new OptionsBuilder().include(NanoKVBenchmark.class.getSimpleName()).build();
        new Runner(opt).run();
    }
    ...
    public static class Context {
        ...
        private NanoKVConfig config1 = NanoKVConfig.from(ConfigFactory.load("node1.conf"));
        private NanoKVConfig config2 = NanoKVConfig.from(ConfigFactory.load("node2.conf"));
        private NanoKVConfig config3 = NanoKVConfig.from(ConfigFactory.load("node3.conf"));
        ...
        @Setup(Level.Trial)
        public void setup() {
            servers.add(NanoKV.bootstrap(config1));
            servers.add(NanoKV.bootstrap(config2));
            servers.add(NanoKV.bootstrap(config3));
