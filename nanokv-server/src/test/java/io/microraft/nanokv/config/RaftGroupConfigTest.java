...
public class RaftGroupConfigTest extends BaseTest {
    ...
    public void when_initialEndpointsPresent_then_shouldCreateConfig() {
        ...
        List<NanoKVEndpointConfig> initialMembers = groupConfig.getInitialEndpoints();
        assertThat(initialMembers).hasSize(3);

        Set<String> nodeIds = initialMembers.stream().map(NanoKVEndpointConfig::getId).collect(toSet());
        assertThat(nodeIds).hasSize(3);
        assertThat(nodeIds).contains("node1", "node2", "node3");

        Set<String> addresses = initialMembers.stream().map(NanoKVEndpointConfig::getAddress).collect(toSet());
    ...
    public void when_duplicateIdExistsInBuilder_then_shouldNotCreateConfig() {
        ...
        NanoKVEndpointConfig endpointConfig2 = NanoKVEndpointConfig.newBuilder().setId("node2")
                .setAddress("localhost" + ":6768").build();
        NanoKVEndpointConfig endpointConfig3 = NanoKVEndpointConfig.newBuilder().setId("node1")
                .setAddress("localhost" + ":6769").build();
