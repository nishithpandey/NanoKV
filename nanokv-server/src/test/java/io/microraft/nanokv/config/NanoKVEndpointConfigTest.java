...
public class NanoKVEndpointConfigTest extends BaseTest {
    ...
    public void when_configStringContainsIdAndAddress_then_shouldCreateConfig() {
        ...
        NanoKVEndpointConfig endpointConfig = NanoKVEndpointConfig.from(config);
    ...
    public void when_builderIsEmpty_then_shouldNotCreateConfig() {
        NanoKVEndpointConfig.newBuilder().build();
    }

    @Test(expected = NanoKVException.class)
    public void when_addressIsMissingInBuilder_then_shouldNotCreateConfig() {
        NanoKVEndpointConfig.newBuilder().setId("node1").build();
    }

    @Test(expected = NanoKVException.class)
    public void when_idIsMissingInBuilder_then_shouldNotCreateConfig() {
        NanoKVEndpointConfig.newBuilder().setAddress("localhost:6767").build();
    }

    @Test
    public void when_builderContainsIdAndAddress_then_shouldCreateConfig() {
        NanoKVEndpointConfig endpointConfig = NanoKVEndpointConfig.newBuilder().setId("node1")
    ...
    public void when_configIsAlreadyCreated_then_shouldNotCreateConfig() {
        NanoKVEndpointConfig.NanoKVEndpointConfigBuilder builder = NanoKVEndpointConfig.newBuilder();
