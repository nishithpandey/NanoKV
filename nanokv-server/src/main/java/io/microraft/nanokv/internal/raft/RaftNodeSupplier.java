...
import static io.microraft.nanokv.internal.di.NanoKVModule.INITIAL_ENDPOINTS_KEY;
import static io.microraft.nanokv.internal.di.NanoKVModule.LOCAL_ENDPOINT_KEY;
/**
 * Creates and contains a RaftNode instance in the NanoKV instance
 */
@Singleton
public class RaftNodeSupplier implements Supplier<RaftNode> {
    ...
    @Inject
    public RaftNodeSupplier(@Named(CONFIG_KEY) NanoKVConfig config,
    ...
    public void start() {
        ...
        try {
            ...
            throw new NanoKVException(raftNode.getLocalEndpoint().getId() + " could not start Raft node!", t);
        }
    }
