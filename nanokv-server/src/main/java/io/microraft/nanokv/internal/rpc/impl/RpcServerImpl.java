...
public class RpcServerImpl implements RpcServer {
    ...
    @Inject
    public RpcServerImpl(@Named(LOCAL_ENDPOINT_KEY) RaftEndpoint localEndpoint, @Named(CONFIG_KEY) NanoKVConfig config,
            KVService kvRequestHandler, RaftService raftService, AdminService adminService,
        ...
        this.server = NettyServerBuilder.forAddress(config.getLocalEndpointConfig().getSocketAddress())
                ...
                .addService((NanoKVClusterServiceImplBase) raftNodeReportSupplier).directExecutor().build();
    ...
    public void start() {
        try {
            ...
            throw new NanoKVException(localEndpoint.getId() + " RpcServer start failed!", e);
        }
    }
