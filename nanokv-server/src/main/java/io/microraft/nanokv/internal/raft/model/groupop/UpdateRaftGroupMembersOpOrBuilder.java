...
package io.microraft.nanokv.internal.raft.model.groupop;

import io.microraft.nanokv.NanoKVException;
import io.microraft.nanokv.internal.raft.model.NanoKVEndpoint;
import io.microraft.nanokv.raft.proto.UpdateRaftGroupMembersOpProto;
import io.microraft.nanokv.raft.proto.UpdateRaftGroupMembersOpProto.MembershipChangeModeProto;
...
public class UpdateRaftGroupMembersOpOrBuilder implements UpdateRaftGroupMembersOp, UpdateRaftGroupMembersOpBuilder {
    ...
    public UpdateRaftGroupMembersOpOrBuilder(UpdateRaftGroupMembersOpProto op) {
        this.op = op;
        op.getMemberList().stream().map(NanoKVEndpoint::wrap).forEach(members::add);
        op.getVotingMemberList().stream().map(NanoKVEndpoint::wrap).forEach(votingMembers::add);
        this.endpoint = NanoKVEndpoint.wrap(op.getEndpoint());
    }
    ...
    public UpdateRaftGroupMembersOpBuilder setMembers(@Nonnull Collection<RaftEndpoint> members) {
        members.stream().map(NanoKVEndpoint::unwrap).forEach(builder::addMember);
        this.members.clear();
    ...
    public UpdateRaftGroupMembersOpBuilder setVotingMembers(Collection<RaftEndpoint> votingMembers) {
        members.stream().map(NanoKVEndpoint::unwrap).forEach(builder::addVotingMember);
    ...
    public UpdateRaftGroupMembersOpBuilder setEndpoint(@Nonnull RaftEndpoint endpoint) {
        builder.setEndpoint(NanoKVEndpoint.unwrap(endpoint));
    ...
    public UpdateRaftGroupMembersOpBuilder setMode(@Nonnull MembershipChangeMode mode) {
        ...
        throw new NanoKVException("Invalid mode: " + mode);
    }
