...
import io.microraft.nanokv.raft.proto.KVSnapshotChunk;
import io.microraft.nanokv.raft.proto.LogEntryProto;
import io.microraft.nanokv.raft.proto.RaftEndpointProto;
import io.microraft.nanokv.raft.proto.RaftGroupMembersViewProto;
    ...
    public Serializer<RaftEndpoint> raftEndpointSerializer() {
        return new Serializer<RaftEndpoint>() {
            ...
            public byte[] serialize(@Nonnull RaftEndpoint element) {
                RaftEndpointProto proto = NanoKVEndpoint.unwrap(element);
                if (proto != null) {
                    return proto.toByteArray();
                }
                throw new IllegalArgumentException("Cannot serialize: " + element.getClass());
            ...
            public RaftEndpoint deserialize(@Nonnull byte[] element) {
                try {
                    return NanoKVEndpoint.wrap(RaftEndpointProto.parseFrom(element));
                } catch (InvalidProtocolBufferException e) {
