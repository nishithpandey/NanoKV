...
package io.microraft.nanokv.internal.raft;
...
import io.microraft.nanokv.cluster.proto.NanoKVClusterServiceGrpc.NanoKVClusterServiceImplBase;
...
import static io.microraft.nanokv.internal.di.NanoKVModule.CONFIG_KEY;
import static io.microraft.nanokv.internal.di.NanoKVModule.LOCAL_ENDPOINT_KEY;
import static io.microraft.nanokv.internal.utils.Exceptions.runSilently;
import static io.microraft.report.RaftNodeReport.RaftNodeReportReason.PERIODIC;
import static java.util.concurrent.TimeUnit.SECONDS;

@Singleton
public class NanoKVClusterEndpointsPublisher extends NanoKVClusterServiceImplBase implements RaftNodeReportSupplier {

    private static final Logger LOGGER = LoggerFactory.getLogger(NanoKVClusterEndpointsPublisher.class);
    private static final long CLUSTER_ENDPOINTS_IDLE_PUBLISH_DURATION_MILLIS = SECONDS.toMillis(30);

    private final Map<String, StreamObserver<NanoKVClusterEndpointsResponse>> observers = new ConcurrentHashMap<>();
    ...
    @Inject
    public NanoKVClusterEndpointsPublisher(@Named(CONFIG_KEY) NanoKVConfig config,
    ...
    @Override
    public void listenClusterEndpoints(NanoKVClusterEndpointsRequest request,
            StreamObserver<NanoKVClusterEndpointsResponse> responseObserver) {
        StreamObserver<NanoKVClusterEndpointsResponse> prev = observers.put(request.getClientId(), responseObserver);
        if (prev != null) {
    ...
    private void publish(RaftNodeReport report) {
        NanoKVClusterEndpointsResponse response = createResponse(report);
        lastReport = report;
        Iterator<Entry<String, StreamObserver<NanoKVClusterEndpointsResponse>>> it = observers.entrySet().iterator();
        while (it.hasNext()) {
            Entry<String, StreamObserver<NanoKVClusterEndpointsResponse>> e = it.next();
            String clientId = e.getKey();
            StreamObserver<NanoKVClusterEndpointsResponse> observer = e.getValue();
            try {
    ...
    private NanoKVClusterEndpointsResponse createResponse(RaftNodeReport report) {
        RaftGroupMembers committedMembers = report.getCommittedMembers();

        NanoKVClusterEndpoints.Builder endpointsBuilder = NanoKVClusterEndpoints.newBuilder();
        ...
        return NanoKVClusterEndpointsResponse.newBuilder().setEndpoints(endpointsBuilder.build()).build();
    }
