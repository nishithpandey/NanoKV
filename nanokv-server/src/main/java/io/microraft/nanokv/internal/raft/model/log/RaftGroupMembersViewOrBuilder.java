...
package io.microraft.nanokv.internal.raft.model.log;
...
public class RaftGroupMembersViewOrBuilder implements RaftGroupMembersView, RaftGroupMembersViewBuilder {
    ...
    public RaftGroupMembersViewOrBuilder(RaftGroupMembersViewProto groupMembersView) {
        ...
        groupMembersView.getMemberList().stream().map(NanoKVEndpoint::wrap).forEach(members::add);
        groupMembersView.getVotingMemberList().stream().map(NanoKVEndpoint::wrap).forEach(votingMembers::add);
    }
    ...
    public RaftGroupMembersViewBuilder setVotingMembers(@Nonnull Collection<RaftEndpoint> votingMembers) {
        votingMembers.stream().map(NanoKVEndpoint::unwrap).forEach(builder::addVotingMember);
