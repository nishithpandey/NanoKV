...
package io.microraft.nanokv.internal;
...
import io.microraft.nanokv.internal.di.NanoKVModule;
...
import io.microraft.nanokv.admin.proto.GetRaftNodeReportResponse;
import io.microraft.nanokv.admin.proto.AdminServiceGrpc;
import io.microraft.nanokv.admin.proto.RaftNodeReportProto;
import io.microraft.nanokv.admin.proto.RaftNodeStatusProto;
...
public class NanoKVImpl implements NanoKV {

    private static final Logger LOGGER = LoggerFactory.getLogger(NanoKV.class);
    ...
    private NanoKVImpl(NanoKVConfig config, RaftEndpoint localEndpoint, List<RaftEndpoint> initialEndpoints,
            Map<RaftEndpoint, String> endpointAddresses) {
        try {
            this.config = config;
            this.localEndpoint = localEndpoint;
            Module module = new NanoKVModule(config, localEndpoint, initialEndpoints, endpointAddresses,
            ...
            shutdown();
            throw new NanoKVException("Could not start server!", t);
        }
    }

    private static RaftEndpointProto toProtoRaftEndpoint(NanoKVEndpointConfig endpointConfig) {
    ...
    public static class NanoKVBootstrapper implements Supplier<NanoKVImpl> {
        final NanoKVConfig config;

        public NanoKVBootstrapper(NanoKVConfig config) {
            this.config = config;
        }

        @Override
        public NanoKVImpl get() {
            ...
            return new NanoKVImpl(config, NanoKVEndpoint.wrap(localEndpoint), initialEndpoints, endpointAddresses);
        }

        private List<RaftEndpoint> getInitialEndpoints(NanoKVConfig config) {
            List<RaftEndpoint> initialEndpoints = config.getRaftGroupConfig().getInitialEndpoints().stream()
                    .map(NanoKVImpl::toProtoRaftEndpoint).map(NanoKVEndpoint::wrap).collect(toList());
            if (initialEndpoints.size() < 2) {
                throw new NanoKVException(
                        "Cannot bootstrap new NanoKV cluster with " + initialEndpoints.size() + " endpoint!");
            }

            return initialEndpoints;
        }

        private Map<RaftEndpoint, String> getEndpointAddresses(NanoKVConfig config) {
            return config.getRaftGroupConfig().getInitialEndpoints().stream().collect(
                    toMap(c -> NanoKVEndpoint.wrap(toProtoRaftEndpoint(c)), NanoKVEndpointConfig::getAddress));
        }

    }

    public static class NanoKVJoiner implements Supplier<NanoKVImpl> {
        ...
        public NanoKVJoiner(NanoKVConfig config, boolean votingMember) {
            this.config = config;
        ...
        public NanoKVImpl get() {
            ...
            if (joinAddress == null) {
                throw new NanoKVException("Join address is missing!");
            }
            ...
            if (reportResponse.getReport().getCommittedMembers().getMemberList().contains(localEndpoint)) {
                ...
                LOGGER.warn(
                        "{} already joined to the Raft group before. NanoKV will be created with initial "
                                + "endpoints: {} and addresses: {}",
                        localEndpoint.getId(), initialMembers, endpointAddresses);

                return new NanoKVImpl(config, NanoKVEndpoint.wrap(localEndpoint), initialMembers, endpointAddresses);
            }
            ...
            LOGGER.info(
                    "{} joined to the Raft group. NanoKV is created with initial endpoints: {} and " + "addresses: {}",
                    localEndpoint.getId(), initialMembers, endpointAddresses);
        ...
        private void verifyReport(String joinAddress, GetRaftNodeReportResponse reportResponse) {
            ...
            for (RaftEndpointProto endpoint : report.getEffectiveMembers().getMemberList()) {
                if (reportResponse.getEndpointAddressOrDefault(endpoint.getId(), null) == null) {
                    throw new NanoKVException("Cannot join via " + joinAddress + " because the address of the Raft "
        ...
        private void broadcastLocalAddress(AddRaftEndpointAddressRequest request, RaftEndpointProto target,
            ...
            try {
                ...
                throw new NanoKVException("Could not add Raft endpoint address to " + target + " at " + targetAddress,
                        t);
            } finally {
                channel.shutdownNow();
            }
        ...
        private void addRaftEndpoint(GetRaftNodeReportResponse reportResponse) {
            ...
            try {
                ...
                throw new NanoKVException(localEndpoint.getId() + " failure during add Raft endpoint via the Raft "
        ...
        private void populateDBInitState(GetRaftNodeReportResponse reportResponse) {
            for (RaftEndpointProto endpoint : reportResponse.getReport().getInitialMembers().getMemberList()) {
                initialMembers.add(NanoKVEndpoint.wrap(endpoint));
            }

            for (RaftEndpointProto endpoint : reportResponse.getReport().getEffectiveMembers().getMemberList()) {
                String address = reportResponse.getEndpointAddressOrDefault(endpoint.getId(), null);
                endpointAddresses.put(NanoKVEndpoint.wrap(endpoint), address);
            }
        }
